...
- Extracts `user` and `assistant` messages — skips `session`, `model_change`, `thinking_level_change`, `compaction`, `toolResult`
- Captures tool calls alongside prose, wrapped in `<tool>...</tool>` (same format as native capture via `formatToolCall`)
- Groups consecutive assistant messages into a single `[Agent]` block (matching native capture behavior)
