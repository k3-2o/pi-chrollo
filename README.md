...
- **Auto-inject** — proximity-based memory injection. Never blocks rendering. Skips trivial prompts ("thanks", "continue") and identical-term repeats so the search budget goes to prompts that need it. Injects up to 10 compact results + `(+N more — use memory intelligently)` heads-up. De-duplicated across follow-up turns of the same topic.
- **`read_memory` tool** — single-pass AND search (all terms must co-occur). Compact output: `path:line | text`. Up to 20 results, capped at 3 per session for diversity. Rare-word matches (e.g. `k3s`) rank higher than common ones (`config`) via IDF weighting.
...
- **Recency scoring** — line-level timestamps, ~30-day half-life so last-month context still ranks. Memories you keep reading stay fresh longer (access-reinforced decay, tracked in `.chrollo/access.json`). Score = `Σ(idf × matchedTerms) × recencyMultiplier`.
